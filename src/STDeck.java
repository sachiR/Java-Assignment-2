...
public class STDeck {
    ...
    private ArrayList<STCard> cards = new ArrayList<STCard>();

    public ArrayList<STCard> getCards() { return cards; }

    public STCard getCard(int index){return cards.get(index);}
    ...
    private  HashMap<String,ArrayList<String>> getCardDetails(Element element){
        ...
        int occurrenceValues = element.getElementsByTagName("string").getLength() - 11;
        int delta = 0;
        ...
        for (int count=0;(count < element.getElementsByTagName("key").getLength() && (!cardType.equals("rule")));count++){
            ...
            if(count>3){
                if (key.equals("occurrence")){
                    for(int temp = 0; temp < occurrenceValues; temp++){
                        values.add(element.getElementsByTagName("string")
                                .item((count-2)+temp)
                                .getTextContent());
                        delta++;
